...
#define REWARD_MULTIPLIER 10.0f //4.0f//10.0f //200.0f
...
void ArmPlugin::Load(physics::ModelPtr _parent, sdf::ElementPtr /*_sdf*/) 
{
	...
	*/
	...
	//cameraSub = None;
	cameraSub =
			cameraNode->Subscribe("/gazebo/" WORLD_NAME "/camera/link/camera/image",
	...
	*/
	...
	//collisionSub = None;
	collisionSub =
			collisionNode->Subscribe("/gazebo/" WORLD_NAME "/" PROP_NAME "/tube_link/my_contact",
	...
	for (unsigned int i = 0; i < contacts->contact_size(); ++i)
	{
		...
		*/
		...
		if ((strcmp(contacts->contact(i).collision1().c_str(), COLLISION_ITEM)
				== 0) && strcmp(contacts->contact(i).collision2().c_str(),
								COLLISION_POINT) == 0)
			//if ((strcmp(contacts->contact(i).collision1().c_str(),COLLISION_ITEM) == 0))
		{
			...
			newReward  = true;
			endEpisode = true;
			if (DEBUG) {std::cout << "XXX collision: COLLISION_ITEM" << std::endl;}
...
bool ArmPlugin::updateAgent()
{
	...
	*/
	...
	float joint = ref[jointIdx]
			+ actionJointDelta * ((action % 2 == 0) ? 1.0f : -1.0f); // TODO - Set joint position based on whether action is even or odd.
...
void ArmPlugin::OnUpdate(const common::UpdateInfo& updateInfo)
{
	...
	if( hadNewState && !newReward )
	{
		...
		const float groundContact = 0.05f;//0.05f;
		...
		*/ 
		...
		if(!checkGroundContact)
		{
			...
			if( episodeFrames > 1 )
			{
				...
				const float movingAvg  = 0.9f;//0.0f;//0.9f;
				const float costPerStep = 0.5f;
				...
				rewardHistory = REWARD_MULTIPLIER * avgGoalDelta - costPerStep;
				newReward     = true;
			}
	...
	if( newReward && agent != NULL )
	{
		...
		if( endEpisode )
		{
			...
			if( totalRuns >= RUN_HISTORY )
			{
				...
				printf("%02u of last %u  (%0.2f)  (max=%0.2f)\n", historyWins, RUN_HISTORY, float(historyWins)/float(RUN_HISTORY), float(runHistoryMax)/float(RUN_HISTORY));
			}
